 ...
 * <li><a href="#lossy">When the requested type is a long, other {@link Number}
 ...
 * fails with a {@code RuntimeException} if the requested name has no value or
...
public final class JSONObject extends JSONCollection implements Map<String, Object> {
     ...
     * <li>return {@code true} for {@link #containsKey(Object)}
     ...
     * returning true when compared to {@code null}. Its {@link #toString} method
     * returns "null".
     */
    public static final Object NULL = new Serializable() {
        ...
        // Serialization magic: after de-serializing, it will be back to the singleton
        // instance of NULL.
    ...
    private static RuntimeException tokenerError(Object actual, JSONType required) {
        if (actual == null)
            return new JSONValueNotFoundException("Value", required);
        return new JSONTypeMismatchException("Value", required, actual.getClass());
    }

    private static RuntimeException typeError(Object name, Object actual, JSONType required) {
        if (actual == null)
            return valueError(name, required);
        return new JSONTypeMismatchException("JSONObject[\"" + name + "\"]", required, actual.getClass());
    }

    private static RuntimeException valueError(Object name, JSONType required) {
        return new JSONValueNotFoundException("JSONObject[\"" + name + "\"]", required);
    }
    ...
    JSONObject(JSONTokener readFrom) {
        ...
        if (object instanceof JSONObject) {
            ...
            throw tokenerError(object, JSONType.OBJECT);
        }
    }
     ...
     * from the given object. Names that aren't present in {@code copyFrom} will be
     * skipped.
     *
     * @param copyFrom
     *            The source object.
     ...
     * Object values should be compatible with {@link #put(String, Object)}. Keys
     * must be strings (toString() will be invoked on each key).
     * <p>
     * Prior to release 5.4, keysAndValues was type String...; changing it to
     * Object... makes it much easier to initialize a JSONObject in a single
     * statement, which is more readable.
     *
     */
     ...
     * @param value
     *            a {@link JSONObject}, {@link JSONArray}, String, Boolean, Integer,
     *            Long, Double, {@link #NULL}, or {@code null}. May not be
     ...
     * They key must not exist, or must be a JSONObject.
     *
     * @param key
    ...
    public JSONObject in(String key) {
        ...
        if (nested != null && !(nested instanceof JSONObject)) {
            throw new IllegalStateException(String.format("JSONObject[%s] is not a JSONObject.", JSONUtils.quote(key)));
        }
    ...
    /**
     * Appends {@code value} to the array already mapped to {@code name}. If this
     * object has no mapping for {@code name}, this inserts a new mapping. If the
     * mapping exists but its value is not an array, the existing and new values are
     * inserted in order into a new array which is itself mapped to {@code name}. In
     * aggregate, this allows values to be added to a mapping one at a time.
     * <p>
     ...
     * @param value
     *            a {@link JSONObject}, {@link JSONArray}, String, Boolean, Integer,
     *            Long, Double, {@link #NULL} or null. May not be
     *            {@link Double#isNaN() NaNs} or {@link Double#isInfinite()
     *            infinities}.
     ...
     * existing mapping for {@code name} is not a {@link JSONArray}, a
     * {@link RuntimeException} will be thrown.
     *
     * @param name
     *            The name of the array to which the value should be appended.
     * @param value
     *            The value to append.
     * @return this object.
    ...
    /**
     * Returns true if this object has no mapping for {@code name} or if it has a
     * mapping whose value is {@link #NULL}.
     *
     * @param name
    ...
    /**
     * Returns the value mapped by {@code name} if it exists and is a boolean or can
     * be coerced to a boolean, or throws otherwise.
     *
     * @param name
    ...
    public boolean getBoolean(String name) {
        ...
        if (object == null)
            throw valueError(name, JSONType.BOOLEAN);
        Boolean result = JSON.toBoolean(object);
        if (result == null)
            throw typeError(name, object, JSONType.BOOLEAN);
        return result;
    }
     ...
     *         {@code defaultValue} if this JSONObject contains no mapping for the
     *         key
     * @throws JSONTypeMismatchException
     *             if the mapping cannot be coerced to a boolean.
     */
    ...
    /**
     * Returns the value mapped by {@code name} if it exists and is a double or can
     * be coerced to a double, or throws otherwise.
     *
     * @param name
     *            The name of the field we want.
     * @return The selected value if it exists.
    ...
    public double getDouble(String name) {
        Object object = opt(name);
        if (object == null)
            throw valueError(name, JSONType.NUMBER);
        Double result = JSON.toDouble(object);
        if (result == null)
            throw typeError(name, object, JSONType.NUMBER);
        return result;
    }
     ...
     * coerced to a long, or throws otherwise. Note that JSON represents numbers as
     * doubles,
     * <p>
     * so this is <a href="#lossy">lossy</a>; use strings to transfer numbers via
     * JSON without loss.
     *
     * @param name
     *            The name of the field that we want.
    ...
    public String getString(String name) {
        Object object = opt(name);
        if (object == null)
            throw valueError(name, JSONType.STRING);
        String result = JSON.toString(object);
        if (result == null)
            throw typeError(name, object, JSONType.STRING);
        return result;
    }
     ...
     *         key
     * @throws JSONTypeMismatchException
     *             if the mapping cannot be coerced to a string.
     */
     ...
     *
     * @param name
     *            The field we want to get.
     * @return The value of the field (if it is a JSONArray.
     * @throws JSONValueNotFoundException
     *             if the mapping doesn't exist
     * @throws JSONTypeMismatchException
     *             if the mapping is not a {@code
     *                                    JSONArray}.
     */
    public JSONArray getJSONArray(String name) {
        Object object = opt(name);
        if (object == null)
            throw valueError(name, JSONType.ARRAY);
        if (object instanceof JSONArray) {
            return (JSONArray) object;
        } else {
            throw typeError(name, object, JSONType.ARRAY);
        }
    }
     ...
     *         key
     * @throws JSONTypeMismatchException
     *             if the mapping cannot be coerced to a JSONArray.
     */
    ...
    public JSONObject getJSONObject(String name) {
        Object object = opt(name);
        if (object == null)
            throw valueError(name, JSONType.OBJECT);
        if (object instanceof JSONObject) {
            return (JSONObject) object;
        } else {
            throw typeError(name, object, JSONType.OBJECT);
        }
    }
     ...
     *         {@code defaultValue} if this map contains no mapping for the key
     * @throws JSONTypeMismatchException
     *             if the mapping cannot be coerced to a JSONObject.
     */
    ...
    /**
     * Returns the set of {@code String} names in this object. The returned set is a
     * view of the keys in this object. {@link Set#remove(Object)} will remove the
     * corresponding mapping from this object and set iterator behaviour is
     * undefined if this object is modified after it is returned.
     * <p>
     * See {@link #keys()}.
     *
    ...
    void print(JSONPrintSession session) {
        ...
        for (String key : keys()) {
            ...
            JSONUtils.printValue(session, nameValuePairs.get(key));
    ...
    /**
     * Returns a Map of the keys and values of the JSONObject. The returned map is
     * unmodifiable. Note that changes to the JSONObject will be reflected in the
     * map. In addition, null values in the JSONObject are represented as
     * {@link JSONObject#NULL} in the map.
     *
    ...
    /**
     * Returns the number of key-value mappings in this JSONObject. If it contains
    ...
    public Object get(Object name) {
        ...
        if (result == null)
            throw valueError(name, JSONType.ANY);
        return result;
    }
     ...
     * @return the value previously mapped by {@code name}, or null if there was no
     *         such mapping.
     */
    @Override
    ...
    /**
     * Returns a {@link Set} view of the keys contained in this JSONObject. The set
     * is backed by the JSONObject, so changes to the map are reflected in the set,
     * and vice-versa.
     *
    ...
    /**
     * Returns a {@link Set} view of the mappings contained in this JSONObject. The
     * set is backed by the JSONObject, so changes to the map are reflected in the
     * set, and vice-versa.
     *
     ...
     * @param value
     *            the non-null value to be merged with the existing value associated
     ...
     * @return the new value associated with the specified key, or null if no value
     *         is associated with the key
     * @throws NullPointerException
     *             if the specified key is null or the value or remappingFunction is
     *             null
     */
    @Override
    public Object merge(String key, Object value,
        ...
        // #get(String) to check for the key, which will throw a {@code
