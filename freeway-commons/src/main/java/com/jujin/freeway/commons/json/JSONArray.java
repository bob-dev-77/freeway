...
import com.jujin.freeway.commons.json.exceptions.JSONArrayIndexOutOfBoundsException;
...
public final class JSONArray extends JSONCollection implements Collection<Object> {
    ...
    private static RuntimeException typeError(Object index, Object actual, JSONType required) {
        if (actual == null)
            return valueError(index, required);
        return new JSONTypeMismatchException("JSONArray[" + index + "]", required, actual.getClass());
    }

    private static RuntimeException valueError(Object index, JSONType required) {
        return new JSONValueNotFoundException("JSONArray[" + index + "]", required);
    }
     ...
     *             if it doesn't yield a {@code JSONArray}.
     */
    JSONArray(JSONTokener readFrom) {
        ...
        if (object instanceof JSONArray) {
            ...
            throw tokenerError(object, JSONType.ARRAY);
        }
    }
     ...
     *             if any of the values are non-finite double values (i.e. NaN or
     *             infinite)
     */
     ...
     * {@link #putAll(Iterable)}.
     * <p>
     * This is implemented as a static method so as not to break the semantics of
     * the existing {@link #JSONArray(Object...)} constructor. Adding a constructor
     * of type Iterable would change the meaning of
    ...
    /**
     * Returns the number of values in this array. If this list contains more than
     ...
     *            Long, Double, or {@link JSONObject#NULL}}. May not be
     *            {@link Double#isNaN() NaNs} or {@link Double#isInfinite()
     *            infinities}. Unsupported values are not permitted and will cause
     *            the array to be in an inconsistent state.
    ...
    /**
     * Sets the value at {@code index} to {@code value}, null padding this array to
     * the required length if necessary. If a value already exists at {@code
    ...
    /**
     * Returns true if this array has no value at {@code index}, or if its value is
     * the {@code null} reference or {@link JSONObject#NULL}.
     *
     * @param index
     ...
     * @throws JSONArrayIndexOutOfBoundsException
     *             if the given index is out of bounds.
     * @throws JSONValueNotFoundException
     *             if this array has no value at {@code index}, or if that value is
     *             the {@code null} reference. This method returns normally if the
     *             value is {@code JSONObject#NULL}.
     */
    public Object get(int index) {
        try {
            Object value = values.get(index);
            if (value == null)
                throw valueError(index, JSONType.ANY);
            return value;
    ...
    /**
     * Removes and returns the value at {@code index}, or null if the array has no
     * value at {@code index}.
     *
     * @param index
    ...
    /**
     * Removes the first occurrence of the specified value from this JSONArray, if
     * it is present.
     *
     * @param value
     *            value to be removed from this JSONArray, if present
     ...
     *
     * @param collection
     *            collection containing value to be removed from this JSONArray
    ...
    /**
     * Retains only the values in this JSONArray that are contained in the specified
     * collection.
     *
     * @param collection
    ...
    /**
     * Returns the value at {@code index} if it exists and is a boolean or can be
     ...
     * @return the value at the specified location.
     * @throws JSONTypeMismatchException
     *             if the value at {@code index} doesn't exist or cannot be coerced
     *             to a boolean.
     */
    public boolean getBoolean(int index) {
        ...
        if (result == null)
            throw typeError(index, object, JSONType.BOOLEAN);
        return result;
    }
    ...
    public double getDouble(int index) {
        ...
        if (result == null)
            throw typeError(index, object, JSONType.NUMBER);
        return result;
    }
    ...
    public String getString(int index) {
        ...
        if (result == null)
            throw typeError(index, object, JSONType.STRING);
        return result;
    }
    ...
    public JSONArray getJSONArray(int index) {
        ...
        if (object instanceof JSONArray) {
            ...
            throw typeError(index, object, JSONType.ARRAY);
        }
    }
    ...
    public JSONObject getJSONObject(int index) {
        ...
        if (object instanceof JSONObject) {
            ...
            throw typeError(index, object, JSONType.OBJECT);
        }
    }
    ...
    void print(JSONPrintSession session) {
        ...
        for (Object value : values) {
            ...
            JSONUtils.printValue(session, value);
    ...
    /**
     * Returns an unmodifiable list of the contents of the array. This is a wrapper
     * around the list's internal storage and is live (changes to the JSONArray
     ...
     *
     * @param array
     *            the array into which the values of this JSONArray are to be
     ...
     *
     * @param value
     *            value whose presence in this JSONArray is to be tested
     * @return {@code true} if this JSONArray contains the specified value
     */
    @Override
    ...
    /**
     * Returns {@code true} if this JSONArray contains all of the values in the
     * specified collection.
     *
     * @param c
