 ...
 * 4627. For legacy reasons this parser is lenient, so a successful parse does
 * not indicate that the input string was valid JSON. All of the following
...
class JSONTokener {
     ...
     *
     * @return a {@link JSONObject}, {@link JSONArray}, String, Boolean, Integer,
     *         Long, Double or {@link JSONObject#NULL}.
    ...
    Object nextValue(Class<?> desiredType) {
        ...
        if (JSONObject.class.equals(desiredType) && c != '{') {
            throw syntaxError(MessageFormat.format("A JSONObject text must start with '''{''' (actual: ''{0}'')",
                Character.toString((char) c)));
        }
        if (JSONArray.class.equals(desiredType) && c != '[') {
            throw syntaxError(MessageFormat.format("A JSONArray text must start with ''['' (actual: ''{0}'')",
                Character.toString((char) c)));
        }
        switch (c) {
            case -1:
    ...
    private String nextString(char quote) {
         ...
         * as a substring of the input. But if we encounter an escape sequence, we need
         * to use a StringBuilder to compose the result.
         */
    ...
    private Object readLiteral() {
        ...
        if (literal.indexOf('.') == -1) {
            ...
            try {
                ...
                /*
                 * This only happens for integral numbers greater than Long.MAX_VALUE, numbers
                 * in exponential form (5e-10) and unquoted strings. Fall through to try
                 * floating point.
                 */
            }
        }
    ...
    /**
     * Returns the string up to but not including any of the given characters or a
     * newline character. This does not consume the excluded character.
     */
    ...
    private JSONObject readObject() {
        ...
        while (true) {
             ...
             * or an arrow "=>". The last two are bogus but we include them because that's
     ...
     * The opening brace '[' should have already been read. Note that "[]" yields an
     * empty array, but "[,]" returns a two-element array equivalent to
     * "[null,null]".
     */
    ...
    /**
     * Returns an exception containing the given message plus the current position
