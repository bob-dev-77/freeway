 ...
 * underlying resources has changed. This class is capable of using either
 * millisecond-level granularity or second-level granularity. Millisecond-level
 * granularity is used by default. Second-level granularity is provided for
 * compatibility with browsers vis-a-vis resource caching -- that's how granular
 * they get with their "If-Modified-Since", "Last-Modified" and "Expires"
 * headers.
 *
 * @param <T>
 *            The type of the optional information about the tracked resource.
 *            This type should implement <code>equals()</code> and
 *            <code>hashCode()</code>.
 */
public class URLChangeTracker<T> {
    ...
    public static final ClassPathURLConverter DEFAULT_CONVERTER = url -> url;

    /**
     * Creates a tracker using the default (does nothing) URL converter, with
     * default (millisecond) granularity and folder tracking disabled.
     *
     */
    ...
    /**
     * Creates a new URL change tracker, using either millisecond-level granularity
     * or second-level granularity and folder checking enabled.
     *
     * @param classpathURLConverter
     *            used to convert URLs from one protocol to another
    ...
    /**
     * Stores a new URL into the tracker, or returns the previous time stamp for a
     * previously added URL. Filters out all non-file URLs.
     *
     * @param url
     ...
     * into the tracker, or returns the previous time stamp for a previously added
     * URL. Filters out all non-file URLs.
     *
     * @param url
     *            of the resource to add, or null if not known
     * @param resourceInfo
     *            an optional object containing information about the tracked URL.
     *            It's returned in the {@link #getChangedResourcesInfo()} method.
     * @return the current timestamp for the URL (possibly rounded off for
     *         granularity reasons), or 0 if the URL is null
     */
    ...
    /**
     * Re-acquires the last updated timestamp for each URL and returns true if any
    ...
    /**
     * Re-acquires the last updated timestamp for each URL and returns the non-null
    ...
    /**
     * Returns the time that the specified file was last modified, possibly rounded
    ...
    private long applyGranularity(long timestamp) {
        // For coarse granularity (accurate only to the last second), remove the
        // milliseconds since
        // the last full second. This is for compatibility with client HTTP requests,
        // which
        // are only accurate to one second. The extra level of detail creates false
        // positives
