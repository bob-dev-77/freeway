...
public class ExceptionAnalyzerImpl implements ExceptionAnalyzer {
    ...
    /**
     * A tuple used to communicate up a lavel both the exception info and the next
     * exception in the stack.
     */
    ...
    /**
     * We want to filter out exceptions that do not provide any additional value.
     * Additional value includes: an exception message not present in the containing
     * exception or a property value not present in the containing exception. Also
     * the first exception is always valued and the last exception (with the stack
     * trace) is valued.
     *
     * @param previousInfo
     * @param info
    ...
    private ExceptionData extractData(Throwable t) {
        ...
        BeanPropertyAdapter adapter = propertyAccess.getAdapter(t);
