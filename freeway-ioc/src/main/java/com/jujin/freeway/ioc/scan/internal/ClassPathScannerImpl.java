...
public class ClassPathScannerImpl implements ClassPathScanner {
     ...
     *
     * @param packagePath
     *            a package path (like a package name, but using '/' instead of '.',
     *            and ending with '/')
     * @param matcher
     *            passed a resource path from the package (or a sub-package),
     *            returns true if the provided path should be included in the
     *            returned collection
     * @return collection of matching paths, in no specified order
     * @throws java.io.IOException
     */
    @Override
    public Set<String> scan(String packagePath, ClassPathMatcher matcher)
        throws IOException {
    ...
    /**
     * Check whether container supports opening a stream on a dir/package to get a
     ...
     * engines that function like this include Weblogic and OC4J. This is not a full
    ...
    /**
     * Variation of {@link Runnable} that throws {@link IOException}. Still think
    ...
    static class Job {
         ...
         */
        final Deque<IOWork> queue = new ArrayDeque<>();
        ...
        void scanURL(final String packagePath, final URL url)
            throws IOException {
        ...
        /**
         * Scan a dir for classes. Will recursively look in the supplied directory and
         * all sub directories.
         *
         * @param packagePath
        ...
        private void scanDirStream(String packagePath, URL packageURL)
            ...
            try {
                ...
                // This can happen for certain application servers (JBoss 4.0.5 for example),
                // that
            ...
            try {
                while (true) {
                    ...
                    if (matcher.matches(packagePath, line)) {
                        ...
                        // This should match just directories. It may also match files that have no
                        // extension;
