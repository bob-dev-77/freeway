...
public class PropertyAccessImpl implements PropertyAccess {
    ...
    private final ConcurrentHashMap<Class, BeanPropertyAdapter> cache =
    ...
    public Annotation getAnnotation(
        ...
        return getAdapter(instance).getAnnotation(
            instance,
            propertyName,
            annotationClass
        );
    ...
    public BeanPropertyAdapter getAdapter(Class forClass) {
        ...
        if (adapter == null) {
            ...
            if (existing != null) adapter = existing;
        }
    ...
    private static class BeanPropertyAdapterImpl
        implements BeanPropertyAdapter
    {
        ...
        BeanPropertyAdapterImpl(Class clazz) {
            ...
            try {
                for (var entry : descriptors.entrySet()) {
                    ...
                    if (pd.getReadMethod() != null) readHandles.put(
                        entry.getKey(),
                        MethodHandles.publicLookup().unreflect(
                            pd.getReadMethod()
                        )
                    );
                    if (pd.getWriteMethod() != null) writeHandles.put(
                        entry.getKey(),
                        MethodHandles.publicLookup().unreflect(
                            pd.getWriteMethod()
                        )
                ...
                for (var entry : publicFields.entrySet()) {
                    fieldVarHandles.put(
                        entry.getKey(),
                        MethodHandles.publicLookup().unreflectVarHandle(
                            entry.getValue()
                        )
                    );
                }
        ...
        public PropertyAdapter getPropertyAdapter(String name) {
            ...
            if (pd != null) return new ReflectionPropertyAdapter(clazz, pd);

            Field f = publicFields.get(name);
            if (f != null) return new ReflectionPropertyAdapter(clazz, f);
        ...
        public Annotation getAnnotation(
            ...
            Field f = publicFields.get(propertyName);
            if (
                f != null && f.isAnnotationPresent(annotationClass)
    ...
    private static class ReflectionPropertyAdapter implements PropertyAdapter {
        ...
        ReflectionPropertyAdapter(Class clazz, PropertyDescriptor pd) {
            ...
            MethodHandle rh = null,
                wh = null;
            try {
                if (readMethod != null) rh =
                    MethodHandles.publicLookup().unreflect(readMethod);
                if (writeMethod != null) wh =
                    MethodHandles.publicLookup().unreflect(writeMethod);
            } catch (IllegalAccessException e) {
                throw new RuntimeException(e);
            }
            this.readHandle = rh;
        ...
        public Object get(Object instance) {
            try {
                if (readHandle != null) return readHandle.invoke(instance);
                if (fieldHandle != null) return fieldHandle.get(instance);
