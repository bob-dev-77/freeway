 ...
 * delegate out logging behavior to a separate object.
 */
public final class MethodLogger {
    ...
    public void entry(MethodInvocation invocation) {
        ...
        buffer.append(
            format("[%s] %s(", ENTER, invocation.getMethod().getName()));

        for (
            int i = 0;
            i < invocation.getMethod().getParameterTypes().length;
            i++) {
            if (i > 0)
                buffer.append(", ");
    ...
    public void exit(MethodInvocation invocation) {
        ...
        buffer.append(
            format("[%s] %s", EXIT, invocation.getMethod().getName()));
     ...
     *
     * @param invocation
     *            identifies method invocation which failed
     * @param t
