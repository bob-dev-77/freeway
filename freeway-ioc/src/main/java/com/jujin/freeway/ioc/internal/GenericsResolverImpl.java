...
/**
 * Implementation copied from Freeway 5.4's GenericUtils (commons package).
 */
...
public class GenericsResolverImpl implements GenericsResolver {
     ...
     */
    public Class<?> extractGenericReturnType(
    ...
    public Class<?> extractGenericFieldType(
        Class<?> containingClass,
        Field field) {
    ...
    /**
     * Determines if the suspected super type is assignable from the suspected sub
     * type.
     *
    ...
    private boolean isAssignableFrom(
        ...
        if (suspectedSuperType instanceof WildcardType) {
            // ? extends Number
            // needs to match all the bounds (there will only be upper bounds or lower
            // bounds
            for (Type t : ((WildcardType) suspectedSuperType).getUpperBounds()) {
                if (!isAssignableFrom(t, suspectedSubType))
                    return false;
            }
            for (Type t : ((WildcardType) suspectedSuperType).getLowerBounds()) {
                if (!isAssignableFrom(suspectedSubType, t))
                    return false;
            }
            return true;
        }
        ...
        while (curType != null && !curType.equals(Object.class)) {
            ...
            if (curClass.equals(suspectedSuperClass)) {
                ...
                if (suspectedSuperType instanceof Class) {
                    ...
                    // They may represent the same class, but the suspectedSuperType is not
                    // parameterized. The parameter
    ...
    public Class<?> asClass(Type actualType) {
        ...
        if (actualType instanceof ParameterizedType) {
            ...
            // The sun implementation returns getRawType as Class<?>, but there is room in
            // the interface for it to be
            // some other Type. We'll assume it's a Class.
            // TODO: consider logging or throwing our own exception for that day when
            // "something else" causes some confusion
        ...
        if (actualType instanceof TypeVariable) {
            ...
            // There is always at least one bound. If no bound is specified in the source
            // then it will be Object.class
        ...
        throw new RuntimeException(
            String.format("Unable to convert %s to Class.", actualType));
    }

    /**
     * Convert the type into a string. The string representation approximates the
     * code that would be used to define the type.
     *
     * @param type
     *            - the type.
    ...
    /**
     * Method to resolve a TypeVariable to its most <a href=
     * "http://java.sun.com/docs/books/jls/third_edition/html/typesValues.html#112582">reifiable</a>
     * form.
     * <p>
     * <p>
     * How to resolve a TypeVariable:<br/>
     * All of the TypeVariables defined by a generic class will be given a Type by
     * any class that extends it. The Type given may or may not be reifiable; it may
     * be another TypeVariable for instance.
     * <p>
     * Consider <br/>
     * <i>class Pair&gt;A,B> { A getA(){...}; ...}</i><br/>
     * <i>class StringLongPair extends Pair&gt;String, Long> { }</i><br/>
     * <p>
     * To resolve the actual return type of Pair.getA() you must first resolve the
     * TypeVariable "A". We can do that by first finding the index of "A" in the
     * Pair.class.getTypeParameters() array of TypeVariables.
     * <p>
     * To get to the Type provided by StringLongPair you access the generics
     * information by calling StringLongPair.class.getGenericSuperclass; this will
     * be a ParameterizedType. ParameterizedType gives you access to the actual type
     * arguments provided to Pair by StringLongPair. The array is in the same order
     * as the array in Pair.class.getTypeParameters so you can use the index we
     * discovered earlier to extract the Type; String.class.
     * <p>
     * When extracting Types we only have to consider the superclass hierarchy and
     * not the interfaces implemented by the class. When a class implements a
     * generic interface it must provide types for the interface and any generic
     * methods implemented from the interface will be re-defined by the class with
     * its generic type variables.
     *
     * @param typeVariable
     *            - the type variable to resolve.
     * @param containingType
     *            - the shallowest class in the class hierarchy (furthest from
     *            Object) where typeVariable is defined.
     * @return a Type that has had all possible TypeVariables resolved that have
     *         been defined between the type variable declaration and the
     *         containingType.
     */
    private Type resolve(TypeVariable typeVariable, Type containingType) {
        ...
        if (!(genericDeclaration instanceof Class)) {
            // It's a method or constructor. The best we can do here is try to resolve the
            // bounds
        ...
        final var stack = new LinkedList<Type>();

        // If you pass a List<Long> as the containingType then the TypeVariable is going
    ...
    /**
     * @param type
     *            - something like List&lt;T>[] or List&lt;? extends T>[] or T[]
     * @param containingType
     *            - the shallowest type in the hierarchy where type is defined.
     * @return either the passed type if no changes required or a copy with a best
     *         effort resolve of the component type.
     */
    private GenericArrayType resolve(
        GenericArrayType type,
        Type containingType) {
    ...
    /**
     * @param type
     *            - something like List&lt;T>, List&lt;T extends Number>
     * @param containingType
     *            - the shallowest type in the hierarchy where type is defined.
     * @return the passed type if nothing to resolve or a copy of the type with the
     *         type arguments resolved.
     */
    private ParameterizedType resolve(
        ParameterizedType type,
        Type containingType) {
        ...
        return modified
            ? create(type.getRawType(), type.getOwnerType(), types)
            : type;
    }

    /**
     * @param type
     *            - something like List&lt;? super T>, List<&lt;? extends T>,
     *            List&lt;? extends T & Comparable&lt? super T>>
     * @param containingType
     *            - the shallowest type in the hierarchy where type is defined.
     * @return the passed type if nothing to resolve or a copy of the type with the
     *         upper and lower bounds resolved.
     */
    ...
    /**
     * @param types
     *            - Array of types to resolve. The unresolved type is replaced in
     *            the array with the resolved type.
     * @param containingType
     *            - the shallowest type in the hierarchy where type is defined.
    ...
    /**
     * @param rawType
     *            - the un-parameterized type.
     * @param ownerType
     *            - the outer class or null if the class is not defined within
     *            another class.
     * @param typeArguments
     *            - type arguments.
     * @return a copy of the type with the typeArguments replaced.
     */
    static ParameterizedType create(
        final Type rawType,
    ...
    /**
     * Find the index of the TypeVariable in the classes parameters. The offset can
     * be used on a subclass to find the actual type.
     *
     * @param typeVariable
    ...
    private static int getTypeVariableIndex(final TypeVariable typeVariable) {
        ...
        final TypeVariable[] typeParameters = typeVariable
        ...
        for (
            ...
            typeArgumentIndex++) {
            // The .equals for TypeVariable may not be compatible, a name check should be
            // sufficient.
